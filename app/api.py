...
def require_api_key(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not provided_key or provided_key.replace('Bearer ', '') != api_key:
...
def process_image():
    ...
    try:
        ...
        return send_file(buffer, mimetype=f"image/{save_format.lower()}", as_attachment=True, download_name=f"api_processed.{save_format.lower()}")
