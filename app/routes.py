...
def get_preview_response(
    session_path, current_step, session_id, original_filename="image"
):
    ...
    if action == "preview_only":
        ...
        try:
            ...
            future = executor.submit(
                process_image_pipeline, img, request.form, wm_image_stream
            )
            ...
            )  # Use JPEG for even faster network transfer during preview
        ...
        try:
            ...
            future = executor.submit(
                process_image_core, img, mode, request.form, wm_image_stream
            )
            processed_img = future.result()
            ...
            return get_preview_response(
                session_path, current_step, session_id, orig_name
            )
        ...
        try:
            ...
            return send_file(
                ...
                as_attachment=True,
                download_name=f"{base_filename}_edited.{save_format.lower()}",
            )
...
def process_batch_background(job_id, session_path, mode, form_data, wm_image_path=None):
    try:
        ...
        image_files = [
            f for f in session_path.iterdir() if f.is_file() and f.name != "wm_logo"
        ]
...
def batch_start():
    ...
    executor.submit(
        process_batch_background, job_id, session_path, mode, form_data, wm_image_path
    )
